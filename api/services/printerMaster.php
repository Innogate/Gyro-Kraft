...
$router->add('POST', '/master/printers/add', function () {
    $jwt = new JwtHandler();
    $handler = new Handler();
    ...
    try {
        $sql = "INSERT INTO printers (name, address, phone_no, whatsapp_no) 
...
$router->add('GET', '/master/printers', function () {
    $jwt = new JwtHandler();
    $handler = new Handler();
    ...
    try {
        ...
        (new ApiResponse(200, "Success", $printers))->toJson();
    } catch (Exception $e) {
        (new ApiResponse(500, "Error: " . $e->getMessage()))->toJson();
    }
});
...
$router->add('POST', '/master/printers/update', function () {
    $jwt = new JwtHandler();
    $handler = new Handler();
    ...
    try {
        $sql = "UPDATE printers SET name = :name, address = :address, phone_no = :phone_no, whatsapp_no = :whatsapp_no WHERE id = :id";
        $stmt = $conn->prepare($sql);
        $stmt->bindValue(':name', $data->name);
        $stmt->bindValue(':address', $data->address);
        $stmt->bindValue(':phone_no', $data->phone_no);
...
$router->add('POST', '/master/printers/delete', function () {
    $jwt = new JwtHandler();
    $handler = new Handler();
    ...
    try {
        $sql = "UPDATE printers SET status = :status WHERE id = :id";
        $stmt = $conn->prepare($sql);
